...
with tab1:
    ...
    with colA:
        st.markdown("**A. Stacked by Science Area within each Project**")
        ...
        fig1a = px.bar(pa.sort_values(["Project","Science Area"]), x="Project", y="Value", color="Science Area")
        fig1a.update_layout(barmode="stack", yaxis_title=y_title, xaxis_title="Project", yaxis=yaxis, height=520, margin=dict(t=40, r=10, b=0, l=10))
        st.plotly_chart(fig1a, use_container_width=True)

    with colB:
        st.markdown("**B. Stacked by Project within each Science Area**")
        ...
        fig1b = px.bar(sa.sort_values(["Science Area","Project"]), x="Science Area", y="Value", color="Project")
        fig1b.update_layout(barmode="stack", yaxis_title=y_title_b, xaxis_title="Science Area", yaxis=yaxis_b, height=520, margin=dict(t=40, r=10, b=0, l=10), legend_title_text="Project")
...
with tab4:
    ...
    if HAS_KALEIDO:
        try:
            fig1a_png = fig1a.to_image(format="png", scale=2)
        except Exception:
            fig1a_png = None
        try:
            fig1b_png = fig1b.to_image(format="png", scale=2)
        except Exception:
            fig1b_png = None
        try:
        ...
        with colA:
            if fig1a_png:
                st.download_button("Download PNG — Project split (by Area within Project)", data=fig1a_png, file_name="project_split_by_area.png", mime="image/png")
            if fig1b_png:
                st.download_button("Download PNG — Area stacks (by Project within Area)", data=fig1b_png, file_name="area_stacks_by_project.png", mime="image/png")
        with colB:
            if fig2_png:
    ...
    if HAS_PPTX and HAS_KALEIDO:
        ...
        if any([fig1a_png, fig1b_png, fig2_png, staff_bar_png]):
            if fig1a_png:
                add_slide_with_image(prs, "A. Project split by Science Area", fig1a_png)
            if fig1b_png:
                add_slide_with_image(prs, "B. Science Area stacks by Project", fig1b_png)
            if fig2_png:
