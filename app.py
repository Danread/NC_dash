...
DEFAULT_DATA_FILE = "NC_analysis_dash.xlsx"  # packaged with the app

st.set_page_config(page_title=APP_TITLE, layout="wide")
st.title(APP_TITLE)
st.caption("Default data is preloaded from **NC_analysis_dash.xlsx**. You can override it by uploading a new file below.")

# ------------------------------- Data loading -------------------------------
def read_default_dataset():
    """Attempt to read the packaged default Excel file (same folder as app.py)."""
...
if df_raw is None:
    ...
    if df_raw is not None:
        st.info(f"Using packaged default data: {DEFAULT_DATA_FILE}")
    else:
...
# Identify value columns (science areas) while excluding id/total columns
...
# Coerce numeric for value columns
df = df_raw.copy()
...
if has_project_col:
    ...
    # Layout B (first column interleaves project headers and staff rows)
...
# Remove total-like science area names if any slipped through
...
# Person totals (for % of person mode)
...
# Tab 1: Split view with radio toggle
with tab1:
...
# Tab 3: Staff workload (select a staff member)
with tab3:
...
with tab4:
    ...
    if proj_df.empty:
        ...
        # Sum hours by person for this project
...
with tab5:
    ...
    if not HAS_PPTX:
        st.info("PowerPoint export requires **python-pptx**. Install with `pip install -U python-pptx`.")
    ...
    # Determine which split view figure to export from Tab 1
