...
with st.sidebar:
    ...
    if "Project Hours" in proj_totals.columns:
        ...
        if pd.isna(max_total_candidate) or not np.isfinite(max_total_candidate):
            max_total = 0.0
        else:
            max_total = float(max_total_candidate)
    else:
        max_total = 0.0
    min_val = 0.0
    default_val = float(st.session_state.get("min_total", 0.0))
    # clamp default into [min_val, max_total]
