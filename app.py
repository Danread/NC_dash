...
DEFAULT_DATA_FILE = "NC_analysis_dash.xlsx"  # included in repo
...
def do_login():
    cookie_name = st.secrets.get("auth_cookie", {}).get("name", "ukceh_nc_labour_cookie")
    cookie_key = st.secrets.get("auth_cookie", {}).get("key", "CHANGE_ME")
    cookie_expiry_days = int(st.secrets.get("auth_cookie", {}).get("expiry_days", 7))
    creds = st.secrets["credentials"]
    authenticator = stauth.Authenticate(
        creds, cookie_name=cookie_name, key=cookie_key, cookie_expiry_days=cookie_expiry_days,
    )
    name, auth_status, username = authenticator.login(fields={'Form name': '🔐 Sign in'})
    if auth_status is False:
        st.error("Username / password incorrect"); st.stop()
    if auth_status is None:
        st.info("Please sign in to access the dashboard."); st.stop()
...
if AUTH_ENABLED:
    try:
        ...
        st.error(f"Authentication misconfigured: {e}")
        st.stop()
else:
    st.sidebar.warning("Login disabled. Add credentials in Secrets to enable authentication.")
...
if has_project_col:
    ...
    # Layout B: first column interleaves project headers and staff
...
# ---------------- Consistent colour map for Science Areas ----------------
areas_in_view = sorted(filtered["Science Area"].dropna().unique().tolist())
palette = px.colors.qualitative.Plotly
# Build a deterministic mapping so the same Science Area has the same colour across tabs
...
# Tab 4 — FIX: ensure all staff labels visible & colour bars by Science Area
with tab4:
    ...
    if proj_df.empty:
        ...
        # Aggregate by Person and Science Area so we can colour by area
        by_psa = proj_df.groupby(["Person","Science Area"], as_index=False)["Hours"].sum()
        by_psa["Person"] = by_psa["Person"].fillna("(Unassigned)")
        # Order people by total hours descending
        ...
        # Value per global setting
        proj_total = float(by_psa["Hours"].sum())
        if st.session_state["global_view_mode"] == "Percent":
            # Percent of the *project's* hours
            by_psa["Value"] = np.where(proj_total>0, by_psa["Hours"]/proj_total*100.0, np.nan)
            x_title = "% of project's hours (stacked by Science Area)"; xaxis = dict(ticksuffix="%"); text_fmt = lambda v: f"{v:.1f}%"
        else:
            by_psa["Value"] = by_psa["Hours"]; x_title = "Hours on selected project (stacked by Science Area)"; xaxis = dict(); text_fmt = lambda v: f"{v:,.0f}"

        # Dynamic height so every person label is visible
        n_people = len(people_order)
        height = max(380, min(1200, 28 * n_people + 140))  # 28 px per row + padding

        # Build stacked horizontal bar: colour by Science Area using consistent mapping
        proj_staff_fig = px.bar(
            by_psa.sort_values(["Person","Science Area"]),
            x="Value", y="Person", color="Science Area", orientation="h",
            color_discrete_map=area_color_map,
            text=by_psa["Value"].map(text_fmt)
        )
        proj_staff_fig.update_layout(
            barmode="stack",
            xaxis_title=x_title, yaxis_title="Staff",
            xaxis=xaxis,
            height=height,
            margin=dict(t=40, r=10, b=10, l=10),
            yaxis=dict(automargin=True, categoryorder="array", categoryarray=people_order)
        )
...
with tab5:
    ...
    if not HAS_KALEIDO:
        st.info("PNG export requires the **kaleido** package (installed on Streamlit Cloud via requirements.txt).")
    if not HAS_PPTX:
        st.info("PowerPoint export requires **python-pptx** (installed via requirements.txt).")
