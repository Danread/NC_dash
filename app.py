...
def current_defaults_dict():
    keys = ["selected_areas", "selected_projects", "selected_people", "hide_ps", "min_total", "global_view_mode"]
...
with st.sidebar:
    ...
    if st.button("🔄 Reset filters to defaults"):
        for k in ["selected_areas", "selected_projects", "selected_people", "hide_ps", "min_total", "global_view_mode"]:
...
with tab4:
    ...
    if HAS_PPTX and HAS_KALEIDO:
        ...
        from pptx.util import Inches
        prs = Presentation()
        title_slide_layout = prs.slides.layout = prs.slide_layouts[5]  # blank
