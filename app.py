...
if has_project_col:
    # Layout A (explicit columns)
    proj_col = cols_lower["project"]
    pers_col = cols_lower["person"] if has_person_col else None
    long = df.melt(id_vars=[proj_col] + ([pers_col] if pers_col else []),
                   value_vars=value_cols, var_name="Science Area", value_name="Hours")
    long = long.rename(columns={proj_col: "Project", (pers_col or "Person"): "Person"})
else:
    # Layout B (first col alternates)
