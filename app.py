...
st.caption("Upload your data; view everything as **percentages** or **absolute hours** with a single toggle. Now includes **NC_type** filtering.")
...
def current_defaults_dict():
    keys = ["selected_areas", "selected_projects", "selected_people", "selected_nc_types",
            "hide_ps", "min_total", "global_view_mode"]
    return {k: st.session_state.get(k) for k in keys}

with st.expander("📦 Accepted layouts", expanded=False):
    st.markdown("- **Layout A**: Columns `Project`, `Person` (optional), `NC_type` (optional), plus one column per science area (numeric).")
    st.markdown("- **Layout B**: First column alternates between project headers (start with digits) and staff rows underneath; `NC_type` may appear as an extra column and will be used if present.")
...
has_nc_col = "nc_type" in cols_lower  # new

# Identify value columns (science areas) while dropping any total/grand total columns and id columns
exclude = set(filter(None, [cols_lower.get("project"), cols_lower.get("person"), cols_lower.get("nc_type")]))
...
# Coerce numeric for area columns + any explicit total column
df = df_raw.copy()
...
if has_project_col:
    ...
    nc_col = cols_lower["nc_type"] if has_nc_col else None
    id_vars = [proj_col] + ([pers_col] if pers_col else []) + ([nc_col] if nc_col else [])
    ...
    if "Person" not in long.columns:
        long["Person"] = None
    if nc_col and nc_col != "NC_type":
        long = long.rename(columns={nc_col: "NC_type"})
    ...
    nc_col = cols_lower.get("nc_type") if has_nc_col else None
    records = []
    ...
    for _, row in df.iterrows():
        ...
        if is_project_id(str(name)):
            ...
            current_nc = (str(row[nc_col]).strip() if (nc_col and not pd.isna(row.get(nc_col, np.nan))) else None)
...
with st.sidebar:
    ...
    # NC_type filter (new)
    nc_all = sorted([x for x in long["NC_type"].dropna().unique().tolist() if str(x).strip() != ""])
    nc_default = st.session_state.get("selected_nc_types", nc_all) or nc_all
    st.multiselect("NC_type (optional)", nc_all, default=[x for x in nc_default if x in nc_all], key="selected_nc_types")
...
# Apply NC_type if chosen (if user leaves empty selection, keep all)
nc_sel = st.session_state.get("selected_nc_types", [])
if nc_sel:
    mask &= long["NC_type"].fillna("__none__").isin(nc_sel)

# Apply people filter (optional)
people_all = sorted(long["Person"].dropna().unique().tolist())
...
k5.metric("NC types in view", f"{filtered['NC_type'].dropna().nunique():,}")
...
with tab4:
    ...
    if HAS_PPTX and HAS_KALEIDO:
        ...
        if any([fig1_png, fig2_png, staff_bar_png]):
            if fig1_png:
                add_slide_with_image(prs, "Project split by Science Area", fig1_png)
            if fig2_png:
